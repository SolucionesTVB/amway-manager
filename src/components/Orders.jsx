...
function esCerrado(o) {
  return o.entregado_tv && o.entregado_cliente && o.pagado_rafa && o.pagado_tv
}
...
function PedidoCard({ o, expanded, setExpanded, selected, toggleSelect, toggle, deleteOrder, openWhatsApp }) {
  const calc       = calcular(o.total)
  const isSelected = selected.has(o.id)
  const cerrado    = esCerrado(o)

  return (
    <Card style={{ border: isSelected ? '1px solid #4ade80' : cerrado ? '1px solid #1a3a1a' : '1px solid #1e3a1e', opacity: cerrado ? 0.8 : 1 }}>
        ...
        <div style={{ borderTop:'1px solid #1e3a1e', padding:'18px' }} className="slide-in">
          ...
          {(o.order_items || []).map((it, idx) => (
            <div key={it.id} style={{
              ...
              background: idx % 2 === 0 ? '#0d1710' : '#111d13', borderRadius:8, marginBottom:6,
            }}>
              <div style={{ flex:1 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))', gap:10, marginBottom:16 }}>
            {STATUS_FIELDS.map(({ field, label }) => (
              <button key={field} onClick={() => toggle(field, o.id, o[field])} style={{
                ...
                border:'1px solid ' + (o[field] ? '#166534':'#2d4a2d'),
                background: o[field] ? '#14532d':'#0d1710',
                cursor:'pointer', color: o[field] ? '#4ade80':'#6b9e6b', fontFamily:'sans-serif', fontSize:13,
              }}>
                <span style={{
...
export default function Orders({ showToast }) {
  ...
  const openWhatsApp = (ordersToSend) => {
    if (ordersToSend.length === 0) { showToast('Selecciona al menos un pedido', 'error'); return }
  ...
  const cardProps = { expanded, setExpanded, selected, toggleSelect, toggle, deleteOrder, openWhatsApp }
  ...
  return (
    <div style={{ padding:28 }} className="fade-in">
      ...
      <p style={{ color:'#6b9e6b', margin:'0 0 20px', fontFamily:'sans-serif', fontSize:14 }}>
        Marcá los pedidos con el checkbox y envialos a Amway por WhatsApp
      </p>

      {/* Panel resumen seleccionados */}
      {calcSel ? (
        <div style={{ background:'#111d13', border:'1px solid #2d4a2d', borderRadius:12, padding:18, marginBottom:20 }}>
          <div style={{ color:'#9dc89a', fontSize:12, fontFamily:'sans-serif', fontWeight:700, marginBottom:12, textTransform:'uppercase', letterSpacing:1 }}>
            Resumen — {selected.size} pedido{selected.size !== 1 ? 's' : ''} seleccionado{selected.size !== 1 ? 's' : ''}
          </div>
          ...
          <button onClick={sendSelected} style={{
            ...
            padding:'14px', cursor:'pointer', fontSize:16, fontFamily:'sans-serif', fontWeight:700,
      ...
      {!loading && visible.length === 0 && (
        <div style={{ textAlign:'center', padding:40, color:'#4a7a4a', fontFamily:'sans-serif' }}>
      ...
      {archivados.length > 0 && (
        <div>
          <button onClick={() => setVerArchivados(!verArchivados)} style={{
            display:'flex', alignItems:'center', gap:8, width:'100%',
            background:'#0d1710', border:'1px solid #1e3a1e', borderRadius:10,
            padding:'12px 16px', cursor:'pointer', color:'#4a7a4a',
            fontFamily:'sans-serif', fontSize:14, marginBottom: verArchivados ? 10 : 0,
          }}>
            <span style={{ transform: verArchivados ? 'rotate(180deg)' : 'none', transition:'0.2s' }}>
              <Icon name="chevron" size={16}/>
            </span>
            📦 Pedidos cerrados / archivados ({archivados.length})
          </button>
          {verArchivados && (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }} className="slide-in">
              {archivados.map(o => <PedidoCard key={o.id} o={o} {...cardProps} />)}
            </div>
          )}
        </div>
      )}
